...
function pripremiPutanjeAplikacije() {
    ...
    server.get("/servis/app/osobe", async (zahtjev, odgovor) => {
        ...
        const brojOsobaPoStranici = parseInt(zahtjev.query['broj']) || 10;
    ...
    server.get("/servis/app/filmovi", async (zahtjev, odgovor) => {
        try {
            ...
            const datumOdSigurno = typeof datumOd === 'string' ? datumOd : null;
            const datumDoSigurno = typeof datumDo === 'string' ? datumDo : null;
            const brojStranica = parseInt(stranica, 10) || 1;
