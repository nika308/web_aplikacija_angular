...
export class RestOsoba {
    ...
    async postOsobaId(zahtjev, odgovor) {
        odgovor.type("application/json");
        odgovor.status(405);
        let poruka = { greska: "zabranjena metoda" };
        odgovor.send(JSON.stringify(poruka));
    }
    async putOsobaId(zahtjev, odgovor) {
        odgovor.type("application/json");
        odgovor.status(405);
        let poruka = { greska: "zabranjena metoda" };
        odgovor.send(JSON.stringify(poruka));
    }
    async deleteOsobaId(zahtjev, odgovor) {
        let osobaID = Number(zahtjev.params['id']);
        if (isNaN(osobaID)) {
            odgovor.status(400).send({ greska: "neispravan ID" });
            return;
    ...
    async getOsobePoStranici(zahtjev, odgovor) {
        odgovor.type("application/json");
    ...
    async postOsobePoStranici(zahtjev, odgovor) {
        odgovor.type("application/json");
        odgovor.status(400);
        let poruka = { greska: "nije implementirano" };
        odgovor.send(JSON.stringify(poruka));
    }
    async putOsobePoStranici(zahtjev, odgovor) {
        odgovor.type("application/json");
        odgovor.status(405);
        let poruka = { greska: "zabranjena metoda" };
        odgovor.send(JSON.stringify(poruka));
    }
    async deleteOsobePoStranici(zahtjev, odgovor) {
        odgovor.type("application/json");
        odgovor.status(405);
        let poruka = { greska: "zabranjena metoda" };
        odgovor.send(JSON.stringify(poruka));
